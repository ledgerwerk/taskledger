...
- Added plan review approval brief with decision status, question status, and structured body heading lint rule

### Quality

- Improved type safety in WorkspaceContentSnapshot and WorktreePathEntry deserialization with round-trip tests
...
- Added optional code review records: `review record/list/show` with git metadata, worker-pipeline integration, and export/import support.
...
- Added release changelog workflow improvements: project-aware context, `--from-task` ranges, `--include-status`/`--fail-on-omitted`, metadata.
- Added optional worker pipeline overlay: `[worker_pipeline]` config, `pipeline show/list/next`, worker-aware context and handoffs.
...
- Added doctor warning diagnostics for stale worker-step checklist and handoff references.
...
- Runtime event logging is now gated behind `[event_logging] enabled = true` and disabled by default; storage reads unchanged.
...
- Plan template and checklist parsing preserve `worker_step` metadata across export, regeneration, and materialization.
- Updated service boundary whitelists for worker-pipeline and doctor scan import sites.

### Fixed

- Fixed architecture documentation drift: exact-match service boundary whitelist, all 41 command groups, regenerated ARCHITECTURE.md.
...
- Documented code review, task export, config commands, promoted sync, and golden-path agent commands across README, docs, skill, API.
...
- Added regression test modules for worker pipeline config, CLI commands, context, handoffs, checklist materialization, guided next-action, and doctor warnings.
...
- Added duplicate code refactoring: extracted shared `task_events`, `git_utils`, `toml_edit`, and `next_action_payload` helper modules.
- Added `scripts/find_code_clones.py` for AST-based exact-duplicate and normalized-window clone detection.
- Expanded pytest suite with markers, workspace fixtures, fake command runner helpers, and tox fast/integration/full profiles.
