...
`task-0030` in two ledgers are expected. Use `taskledger ledger adopt --from feature-a task-0030` to copy branch-local task history into the current ledger.
Task-first workflow
\-------------------
...
Taskledger stores task-local work state and opaque links.
Cross-ledger semantics belong to an organizer such as ledgerdeck.
