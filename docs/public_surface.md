...
The supported implementation lifecycle includes `implement restart --summary "..."` when a task is in `failed_validation` and `implement resume --reason "..."` when an existing running implementation run has lost its lock.
