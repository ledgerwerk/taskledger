...
New projects use `.ledger/ledger.toml`, `.ledger/ledger.local.toml`, and `.ledger/task/config.toml`. The local workspace provider is `sibling-ledger`; authoritative data resolves to `<project-root>/../ledger/task/taskledger/<project-uuid>`, while indexes remain checkout-scoped cache data. Use `taskledger init --create-sibling-store` when the fixed sibling store needs explicit creation. For migration, `--sibling-ledger-root PATH` supplies the base store and data is isolated below `PATH/task/taskledger/<project-uuid>`. Resolve `data` and `indexes` through `taskledger storage where` or `taskledger storage path`.
