...
New projects use `.ledger/ledger.toml`, `.ledger/ledger.local.toml`, and `.ledger/task/config.toml`. The local workspace provider is `sibling-ledger`; the shared sibling base is `<project-root>/../ledger`, and Taskledger authoritative data resolves to `<project-root>/../ledger/taskledger/<project-uuid>`, while indexes remain checkout-scoped cache data. Other registered ledgers reuse the project UUID and use direct `<ledger-name>/<project-uuid>` directories. Use `taskledger init --create-sibling-store` when the sibling store needs explicit creation. For migration, `--sibling-ledger-root PATH` supplies the base store and data is isolated below `PATH/taskledger/<project-uuid>`. Resolve `data` and `indexes` through `taskledger storage where` or `taskledger storage path`.
