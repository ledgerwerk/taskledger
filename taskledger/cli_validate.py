...
def register_validate_v2_commands(app: typer.Typer) -> None:
    ...
    def import_bdd_report_command(
        ...
        emit_payload(
            ctx,
            payload,
            human=_render_bdd_report_import(payload),
        )


def _render_bdd_report_import(payload: dict[str, Any]) -> str:
    """Render BDD report import result to human-readable text."""
    lines = [
        f"BDD report imported: {payload.get('report_id', '?')}",
        f"Format: {payload.get('format', '?')}",
        f"Result: {payload.get('result', '?')}",
    ]
    matched = payload.get("matched_examples", [])
    if matched:
        lines.append(f"Matched examples: {', '.join(matched)}")
    unmatched = payload.get("unmatched_scenarios", [])
    if unmatched:
        lines.append(f"Unmatched scenarios: {', '.join(unmatched)}")
    checks = payload.get("validation_checks", [])
    if checks:
        lines.append(f"Validation checks created: {len(checks)}")
    return "\n".join(lines)
